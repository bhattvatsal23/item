...
<body id="body-item" class="page-body<?php if($content_only == 1) { ?> content_only<?php } ?><?php if ($ios) { ?> ios<?php } ?>">
        ...
        <?php if($content_only == 0) { ?>
          ...
          <div class="is767">
            <div id="item-basics" class="is_detail">
              ...
              <?php is_seller_online(osc_user_id()); ?>
 <?php if($item_extra['i_condition'] <> '') { ?>
              ...
              <?php if($item_extra['i_transaction'] <> '') { ?>
                ...
                <div class="transaction"><?php _e('Want to', 'veronika'); ?>: <span><?php echo $transaction; ?></span></div>
              <?php } ?>
            ...
            <form action="<?php echo osc_base_url(true) ; ?>" method="post" name="contact_form" id="contact_form" <?php if(osc_item_attachment()) { echo 'enctype="multipart/form-data"'; };?>>
              ...
              <fieldset>
                ...
                <div class="message-block">
                  <?php if( osc_item_is_expired () ) { ?>
                    <div class="empty">
                      <?php _e('This book ad is expired, you cannot contact seller.', 'veronika') ; ?>
                    </div>
                  ...
                  <?php } else if( osc_reg_user_can_contact() && !osc_is_web_user_logged_in() ) { ?>
                    <div class="empty">
                      <?php _e('You must log in or register a new ClassiFind account in order to contact the Book seller.', 'veronika') ; ?>
                    </div>
                  <?php } else { ?>
                    <div class="row first">
                    ...
                    <?php if(osc_item_attachment()) { ?>
                      <div class="attachment">
                        ...
                        <div class="text"><?php _e('Maximum size:', 'veronika'); ?> <?php echo round(osc_max_size_kb()/2000, 1); ?>Mb.</div>
                      </div>
                    <?php } ?>
                    ...
                    <button type="<?php echo (osc_get_preference('forms_ajax', 'veronika_theme') == 1 ? 'button' : 'submit'); ?>" class="send" id="item-message"><i class="fa fa-envelope"></i> <?php _e('Send message from ClasssiFind', 'veronika') ; ?></button>
                  <?php } ?>
                </div>
        ...
        <div id="more-info" class="comments round3 i-shadow is_detail">
          <?php if( osc_comments_enabled()) { ?>
            <div class="item-comments">
              ...
              <div class="care">
                <div class="text"><?php _e('Ask questions related to this posted Book only.', 'veronika'); ?></div>
    ...
    <?php if($content_only == 0) { ?>
      <div id="side-right">
        ...
        <div id="price" class="round3 i-shadow<?php if(function_exists('multicurrency_add_prices') && osc_item_price() <> '' && osc_item_price() > 0) { ?> mc<?php } ?>">
          ...
          <span class="long-price-fix" title="<?php echo osc_esc_html(osc_item_formated_price()); ?>"><?php echo osc_item_formated_price(); ?></span>
        </div>
        ...
        <?php if( !osc_item_is_expired() && (osc_logged_user_id() <> osc_item_user_id() || osc_logged_user_id() == 0 ) && ( !osc_reg_user_can_contact() || osc_is_web_user_logged_in() )) { ?>
          <div class="contact-button btn btn-primary tr1 round3 i-shadow<?php if($mobile_found) { ?> mobile-found<?php } ?>">
            ...
            <span class="top"><?php _e('Contact book seller', 'veronika') ; ?></span>
        ...
        <div class="veronika-wrapper btn btn-primary tr1 round3 i-shadow" style="    ">
          <?php hide_phone_show_phone(); ?>
        </div>
              ...
              <?php } else { ?>
                ...
                <div class="elem regdate">
                  <?php if(osc_item_user_id() <> 0) { ?>
                    ...
                    <?php if(isset($get_user['dt_reg_date']) AND $get_user['dt_reg_date'] <> '') { ?>
                      <?php echo __('Joined ClassiFind on', 'veronika') . ' ' . osc_format_date( $get_user['dt_reg_date'] ); ?>
                    <?php } else { ?>
                      <?php echo __('Unknown registration date', 'veronika'); ?>
                    <?php } ?>
                  <?php } else { ?>
        ...
        <div id="location" class="round3 i-shadow">
          <h2 class="sc-click">
            <?php _e('Book Selling location', 'veronika') ; ?>
          </h2>
